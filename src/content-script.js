(() => {
	...
	if(shouldSpoof) EnterKioskMode();

	if(shouldSpoof) setInterval(() => {
		if((screen.height - innerHeight > 5) || (screen.width - innerWidth > 5))
			WhoopsieDaises();
	...
	function ButtonAction() {
		...
		// url.hash = 'gfu';
		// open(url.href, null, 'location=false,toolbar=false,menubar=false,status=false,popup=true,width='+(window.outerWidth-80)+',height='+(window.outerHeight-80));
	}
	...
	`)
		...
		const OfficialButton = Form.childNodes[2].children[0];
		OfficialButton.style.backgroundColor = '';
		OfficialButton.children[2].children[0].onclick = ButtonAction;
		//.innerText = 'Test'
